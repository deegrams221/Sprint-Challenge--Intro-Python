...
c = [human.name for human in humans if human.name[0] <= "G" and human.name >= "C"]
                            # if <= "G" is after >= "C" then it does not print "Glenn"
print(c)
...
e = [human.name + "-" + str(human.age) for human in humans]
    # use str() for age bc you can not concatenate str & int, make int into a str
print(e)
...
f = [(human.name, human.age) for human in humans if 27 <= human.age <= 32]
    # (str, int) will return ("string", int)
print(f)
...
h = [human.age ** (1/2) for human in humans]
        # using ** 2 squares an int, ** (1/2) square roots an int
print(h)
